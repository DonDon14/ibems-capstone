...
public function storeDetailsData(RequestInterface $request, ResponseInterface $response, int $storeId): ResponseInterface
    {
        ...
        $normalizeDaySession = function (array $row) use ($db, $varianceCases, $expectedService): array {
            ...
            return [
                ...
                'cash_sales' => (float) ($liveExpected['cash_sales'] ?? 0),
                'ecash_sales' => (float) ($liveExpected['ecash_sales'] ?? 0),
                'debt_sales' => (float) ($liveExpected['debt_sales'] ?? 0),
                'cash_in' => (float) ($liveExpected['cash_in'] ?? 0),
                'cash_out' => (float) ($liveExpected['cash_out'] ?? 0),
                'ecash_in' => (float) ($liveExpected['ecash_in'] ?? 0),
                'ecash_out' => (float) ($liveExpected['ecash_out'] ?? 0),
                'payment_method_sales' => array_values($liveExpected['payment_method_sales'] ?? []),
                'payment_method_collections' => array_values($liveExpected['payment_method_collections'] ?? []),
                'payment_account_balances' => array_values($liveExpected['payment_account_balances'] ?? []),
                'unassigned_payment_balances' => array_values($liveExpected['unassigned_payment_balances'] ?? []),
                'cash_sales' => (float) ($liveExpected['cash_sales'] ?? 0),
                'ecash_sales' => (float) ($liveExpected['ecash_sales'] ?? 0),
                'debt_sales' => (float) ($liveExpected['debt_sales'] ?? 0),
                'cash_in' => (float) ($liveExpected['cash_in'] ?? 0),
